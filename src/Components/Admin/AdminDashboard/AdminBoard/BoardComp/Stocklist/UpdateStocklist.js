...
const UpdateStocklist = () => {
  ...
  const getStock = async () => {
    ...
    const URL = `${liveURL}/api/stocklist/${id}`;
  ...
  const formSummit = handleSubmit(async (value) => {
    ...
    const mainURL = "http://localhost:2221";
    const liveURL = "https://skintriumph-server.herokuapp.com";
    const URL = `${liveURL}/api/stocklist/update/${id}`;

    await axios
      .patch(URL, { office, phoneNo, address, email, hours })
      .then((res) => {
        console.log(res.data.data);
        Swal.fire({
          position: "center",
          icon: "success",
          title: "Stocklist Updated",
          showConfirmButton: false,
          timer: 2500,
        }).then(() => {
        ...
        <Card>
          ...
          <StoreHold onSubmit={formSummit}>
            ...
            <InputCtrl>
              ...
              <input
                defaultValue={stockData.phoneNo}
            ...
            <Button>
              <button type="submit">Update Stocklist</button>
            </Button>
          </StoreHold>
